...
RSpec.describe "Inventories", type: :request do
  ...
  describe 'GET :user_id/inventories/' do
    let(:user) {
    ...
    let(:item) { Item.create!(name: 'água', cost: 99) }

    it 'shows the user inventory' do
      inventory.items << item

      get "/api/v1/users/#{user.id}/inventory"
      expect(response.status).to eq 200
      expect(response.body).to eq({items: [{name: "água", cost: 99}]}.to_json)
    end
  end
end
